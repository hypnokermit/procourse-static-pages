    ...
    <div class="customize procourse-static-pages admin admin-customize">
      <div class="admin-container">
        <div class="content-list span6">
          ...
          <ul>
            {{#each @controller.model.items as |page|}}
              <li><button
                  type="button"
                  {{on "click" (fn @controller.selectPCPage page)}}
                  class={{if page.selected "active"}}
                >{{page.title}}</button></li>
            {{/each}}
          </ul>
          {{#unless @controller.model.items.length}}
            <p class="desc">{{i18n
        ...
        {{#if @controller.selectedItem.selected}}
          <div class="show-current-style current-style color-scheme">
            <div class="admin-container">
              <h1><DTextField
                  @class="style-name"
                  @value={{@controller.selectedItem.title}}
                  {{on "input" @controller.editTitle}}
                /></h1>
              <div class="controls">
                <button
                ...
                <button
                  ...
                  disabled={{@controller.disableEnable}}
                  class="btn"
                  type="button"
                >
                ...
                <button
                  {{on "click" @controller.copyPage}}
                  class="btn"
                  type="button"
                >{{dIcon "copy"}}
                  {{i18n "admin.customize.copy"}}</button>
                <button
                  {{on "click" @controller.deletePage}}
                ...
                <span
                  class="saving
                    {{unless @controller.selectedItem.savingStatus 'hidden'}}"
                >{{@controller.selectedItem.savingStatus}}</span>
              </div>
            </div>
            <br />
            <form class="form-horizontal">
              {{#if @controller.selectedItem.id}}
                <div>
                  ...
                  <p class="help">{{i18n
                    ...
                    <a
                      target="_blank"
                      href="{{@controller.pageURL}}{{@controller.selectedItem.slug}}/{{@controller.selectedItem.id}}/"
                      rel="noopener noreferrer"
                    >{{@controller.pageURL}}{{@controller.selectedItem.slug}}/{{@controller.selectedItem.id}}/</a></p>
                </div>
              {{/if}}
              <div>
                <label>{{i18n
                    "admin.procourse_static_pages.pages.slug"
                  }}</label>
                <DTextField
                  @class="style-name"
                  @value={{@controller.selectedItem.slug}}
                  @enter={{@controller.editSlug}}
                />
                <p class="help">{{i18n
              ...
              <div>
                ...
                <ComboBox
                  @content={{@controller.customGroups}}
                  @valueAttribute="id"
                  @value={{@controller.selectedItem.group}}
                  @none="admin.procourse_static_pages.groups.bulk_select"
                />
                {{#if @controller.selectedItem.group}}
                  <DButton
                    class="btn-link clear-group-btn"
                    @icon="xmark"
                    @title="admin.procourse_static_pages.groups.clear"
                    @action={{@controller.clearLimitGroup}}
                  />
                {{/if}}
                <p class="help">{{i18n
              ...
              <div>
                <label>
                  {{Input
                    type="checkbox"
                    name="html"
                    checked=@controller.selectedItem.html
                  }}
              ...
              <div class="pcsp-ace">
                {{#if @controller.selectedItem.html}}
                  <AceEditor
                    @content={{@controller.selectedItem.html_content}}
                    @onChange={{fn (mut @controller.selectedItem.html_content)}}
                    @editorId="html|common"
                    @mode="html"
                  />
                {{else}}
                  <DEditor
                    @value={{@controller.selectedItem.raw}}
                    @class="raw-bio"
                  />
                {{/if}}
              </div>
